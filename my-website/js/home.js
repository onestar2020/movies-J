...
async function getTrendingTrailerAndPlay() {
  try {
    const trendingRes = await fetch(`https://api.themoviedb.org/3/trending/all/week?api_key=${API_KEY}`);
    const trendingData = await trendingRes.json();

    const randomItem = trendingData.results.find(item => item.media_type === 'movie' || item.media_type === 'tv');
    if (!randomItem) return;

    const mediaType = randomItem.media_type;
    const mediaId = randomItem.id;
    const title = mediaType === 'movie' ? randomItem.title : randomItem.name;

    const videoRes = await fetch(`https://api.themoviedb.org/3/${mediaType}/${mediaId}/videos?api_key=${API_KEY}`);
    ...
    const container = document.getElementById('banner-video-container');
    container.innerHTML = ''; // Clear old video
    container.appendChild(iframe);

    document.getElementById('banner-title').textContent = title;
  } catch (error) {
    console.error('Failed to fetch banner trailer:', error);
  }
}
