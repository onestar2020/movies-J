  ...
  tmdbResults.forEach(item => {
    ...
    div.innerHTML = `
      <div style="text-align:center">
        <img src="${IMG_URL + item.poster_path}" alt="${item.title || item.name}" style="width:120px;border-radius:5px;cursor:pointer" onclick="showDetails(${JSON.stringify(item).replace(/"/g, '&quot;')})">
        <p style="margin:5px 0"><strong>${item.title || item.name}</strong></p>
        ${item.overview ? `<p style="font-size:12px">${item.overview.slice(0, 100)}...</p>` : ''}
        ${item.vote_average ? `<p style="color:gold">${'★'.repeat(Math.round(item.vote_average / 2))}</p>` : ''}
      </div>
    `;
    container.appendChild(div);
  });

  // === My Uploaded Movies Results ===
  const uploadedMatches = uploads.filter(upload =>
    upload.title.toLowerCase().includes(query.toLowerCase())
  );

  for (const upload of uploadedMatches) {
    const res = await fetch(`${BASE_URL}/search/movie?api_key=${API_KEY}&query=${encodeURIComponent(upload.title)}`);
    const data = await res.json();
    ...
    div.innerHTML = `
      <div style="text-align:center">
        ...
        <p style="font-size:11px;color:#888">📁 My Upload</p>
      </div>
    `;
    container.appendChild(div);
  }
...
// Remaining functions (showUploadModal, playUploadedVideo, etc.) remain unchanged

// Keep your existing implementations of showUploadModal, playUploadedVideo, etc. below...
