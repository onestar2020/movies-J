// js/home.js (FINAL SECURE VERSION)
...
async function fetchTrending(type) {
  // BINAGO: Tumatawag na sa Netlify function
...
// --- ANG IBA PANG BAHAGI NG CODE AY HINDI NA BINAGO, KOPYAHIN MO LANG ITO NG BUO ---
...
function displayItems(containerId, items) {
  ...
  items.slice(0, moviesPerPage).forEach(item => {
    ...
    card.innerHTML = `
      <img src="${posterPath}" alt="${title}" onclick="goToMovie('${item.id}', '${item.media_type}')">
      <p>${title}</p>
    `;
  ...
  results.filter(r => r.media_type !== 'person' && r.poster_path).slice(0, 10).forEach(item => {
    ...
    resultDiv.innerHTML = `
      ...
      <div>
        ...
        <p>${item.release_date ? item.release_date.substring(0, 4) : ''} - ${item.media_type.toUpperCase()}</p>
      </div>
    `;
...
function updateBanner(item) {
  ...
  banner.style.backgroundImage = `linear-gradient(to right, rgba(17, 17, 17, 1) 0%, rgba(17, 17, 17, 0) 100%), url(${IMG_URL}${item.backdrop_path})`;
  bannerTitle.textContent = item.title || item.name;
  
  const playButton = banner.querySelector('.play-btn');
  if(playButton) {
      playButton.onclick = () => goToMovie(item.id, item.media_type);
  }
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (prevBtn && nextBtn) {
    prevBtn.addEventListener('click', () => {
      ...
      updateBanner(bannerItems[bannerIndex]);
    });
